...
import com.shopbee.user.dto.PhoneDTO;
...
public class UserServiceImpl implements UserService {
    ...
    private Address findAddressByUserIdAndAddressId(String tenantId, String userId, String addressId) {
        ...
        if (address == null) {
            ...
            throw new ResourceNotFoundException("ADDRESS_NOT_FOUND", "Address not found: " + addressId);
        }
    ...
    private void validateNewUsername(String tenantId, String username) {
        if (isUsernameExisted(tenantId, username)) {
            throw new ConflictResourceException("USERNAME_EXISTED", "Username already existed: " + username);
        }
    }
    ...
    private void validateUpdatePhone(String tenantId, String userId, PhoneDTO phoneDTO) {
        if (phoneDTO != null) {
            ...
            if (StringUtils.isBlank(countryCode)) {
                throw new InvalidDataException("MISSING_DATA", "Country code is required");
            }
            String number = phoneDTO.getNumber();
            if (StringUtils.isBlank(number)) {
                throw new InvalidDataException("MISSING_DATA", "Phone number is required");
            }
    ...
    private String saveUser(User user) {
        try {
            ...
            throw new TechnicalException("USER_SAVE_FAILED", "Failed to save user: " + e.getMessage(), e);
        }
    }
    ...
    private ResourceNotFoundException userNotFoundException(String userId) {
        return new ResourceNotFoundException("USER_NOT_FOUND", "User not found: " + userId);
    }

    private ConflictResourceException emailExistedException(String email) {
        return new ConflictResourceException("EMAIL_EXISTED", "Email linked to another user: " + email);
    }

    private ConflictResourceException phoneExistedException(String countryCode, String number) {
        return new ConflictResourceException("PHONE_EXISTED", "Phone linked to another user: " + countryCode + number);
    }
}
